/*
 * levels09-10.js — 第 9 關「開闢航道」(network)、第 10 關「艦隊指揮官」(compose 終局戰)
 */
(function (root) {
  'use strict';
  var h = root.DG.h;
  ...
  root.DG.registerLevel({
    ...
    story: [
      ...
      '（小聲說：它們現在都停在<b>預設 bridge</b> 水域……那裡的貨櫃互相叫不出名字。）'
    ],
    teach: {
      title: '自訂 network 與容器名 DNS',
      html: '<p>不指定 network 時，容器都掛在<b>預設 bridge</b>：能上網，但<b>沒有容器名 DNS</b>——' +
        '用名字互找會失敗（<code>bad address</code>）。</p>' +
        '<p><code>docker network create <名字></code> 開一條自訂航道；run 時用 ' +
        '<code>--network <名字></code> 加入。</p>' +
        '<p>同一條自訂 network 上，Docker 內建 DNS 讓你<b>直接用容器名當主機名</b>：' +
        'web 連 <code>db:5432</code> 就通，IP 換了也不怕。</p>',
      map: '<b>港口比喻</b>：自訂 network＝有無線電頻道的專屬航道——同航道的船互喊名字就能對上；預設 bridge 是沒頻道的公用水域。'
    },
    outro: '要讓容器互相叫得出名字：開自訂 network、大家 --network 加入——名字就是地址。',
    setup: function (ctx) {
      var e = ctx.engine;
      e.pull('harbor-db');
      e.pull('webapp');
    ...
    objectives: [
      ...
      { text: '開闢專屬航道：<code>docker network create harbor-net</code>',
        hints: ['network 的子指令 create，航道名用 harbor-net。',
      ...
      { text: '把 <code>db</code> 和 <code>web</code> 都搬進 harbor-net（先拆舊的，再帶 <code>--network</code> 重跑）',
        hints: ['舊容器不會自己換航道：docker rm -f db web，然後各自重跑。',
      ...
      { text: '再試一次：<code>docker exec web ping db</code>——這次名字應該喊得應',
        ...
        onDone: function (ctx) {
          ctx.stage.caption('通了！同一條航道上，容器名就是地址——這就是 Docker 的內建 DNS。', 5200);
  ...
  var BLANKS = {
    ...
    webNet:    { expect: 'fleet-net', why: '第 9 關的教訓：web 和 db 要同一條航道，容器名 DNS 才通——fleet-net。' },
  ...
  function ymlHTML() {
    ...
    return '<span class="yc"># docker-compose.yml — 鯨魚港艦隊調度令</span>\n' +
  ...
  function mountComposeDesk(ctx) {
    ...
    function complete() {
      feedback('調度令完成！到終端機下達 docker compose up -d ——全艦隊出航！', true);
  ...
  root.DG.registerLevel({
    ...
    terminal: true,
    story: [
      '見習生——不，<b>準指揮官</b>。今天是你的結業之戰。',
      '一個服務要 web、要 db、要管線、保險庫、航道……每次都手打一長串指令？艦隊不是這樣帶的。',
      '<b>compose</b> 讓你把整支艦隊寫成一份「調度令」（YAML），一個指令、全員出航。把它組出來吧——每一格都是你學過的東西。'
    ],
    teach: {
    ...
    outro: '從此你不再指揮單一貨櫃，而是宣告整支艦隊——這就是 compose，也是你的新軍階。',
    setup: function (ctx) {
    ...
    objectives: [
      ...
      { text: '下達總攻擊令：<code>docker compose up -d</code>',
